...
{
    // Bazel runfiles paths vary by version; try several patterns.
    std::vector<std::string> candidates = {
        "external/quickfix/spec/FIX42.xml",
        "../+_repo_rules2+quickfix/spec/FIX42.xml",
    };
    // Also search the runfiles directory alongside the binary.
    for (const auto& c : candidates) {
        if (std::ifstream(c).good())
            return c;
    }
    // Fallback: search the runfiles tree.
    namespace fs = std::filesystem;
    for (auto& entry : fs::recursive_directory_iterator(".")) {
        if (entry.path().filename() == "FIX42.xml" &&
            entry.path().string().find("quickfix") != std::string::npos)
            return entry.path().string();
    }
    throw std::runtime_error("Could not locate FIX42.xml dictionary in runfiles");
}
...
public:
    ...
    {
        auto msg = qfBuildRawMessage("FIX.4.2", {
            {35, "A"},
            {49, sender},
            {56, target},
                     ...
                     const std::string& target = "ACCEPTOR")
    {
        ...
        sendRaw(qfBuildRawMessage("FIX.4.2", fields));
    }
...
{
    std::string path = "/tmp/qf_bench_" + std::to_string(port) + ".cfg";
